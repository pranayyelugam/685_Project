...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
    qc_ids, ac_ids, question, qidstr, ans = data
    qa_nodes = set(qc_ids) | set(ac_ids)
    extra_nodes = set()
    for qid in qa_nodes:
        for aid in qa_nodes:
            if qid != aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[qid]) & set(cpnet_simple[aid])
    extra_nodes = extra_nodes - qa_nodes
    return (sorted(qc_ids), sorted(ac_ids), question, sorted(extra_nodes), qidstr, ans)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    qc_ids, ac_ids, question, extra_nodes, qidstr, ans = data
    cid2score = get_LM_score(qc_ids+ac_ids+extra_nodes, question)
    return (qc_ids, ac_ids, question, extra_nodes, cid2score, qidstr, ans)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part3(data):
    qc_ids, ac_ids, question, extra_nodes, cid2score, qidstr, ans = data
    schema_graph = qc_ids + ac_ids + sorted(extra_nodes, key=lambda x: -cid2score[x]) #score: from high to low
    arange = np.arange(len(schema_graph))
    qmask = arange < len(qc_ids)
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
    adj, concepts = concepts2adj(schema_graph)
    return {'adj': adj, 'concepts': concepts, 'qmask': qmask, 'amask': amask, 'cid2score': cid2score, 'qid': qidstr, 'ans': ans}
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes, is_dev=False, debug=False):
    """
    This function will save
        (1) adjacency matrics (each in the form of a (R*N, N) coo sparse matrix)
        (2) concepts ids
    ...
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
        ...
        lines_ground = lines_ground[:20000]
        if debug:
            lines_ground = lines_ground[0:5]
        
        # print(len(lines_ground), len(lines_state))
        ...
        if is_dev:
            ...
            for j, line in enumerate(lines_state):
                dic = json.loads(line)
                num_answers = len(dic['answers']['raw'])
                answer_count += num_answers
                ground_line = lines_ground[answer_count-1]
                ground_dic = json.loads(ground_line)
                assert ground_dic['qid'] == dic['metadata']['id']
                q_ids = set(concept2id[c] for c in ground_dic['qc'])
                QAcontext = "{} {}.".format(ground_dic['sent'], '')
                qid = ground_dic['qid']
                qa_data.append((q_ids, set(), QAcontext, qid, ''))
            ...
            for j, line in enumerate(lines_ground):
                ...
                if is_dev:
                    ...
                    q_ids = q_ids - a_ids
                QAcontext = "{} {}.".format(dic['sent'], ans)
                qid = dic['qid']
                qa_data.append((q_ids, a_ids, QAcontext, qid, ans))
