...
def ground_qa_pair(qa_pair):
    ...
    return {"sent": s, "ans": a, "qc": question_concepts, "ac": answer_concepts, "qid": qid}
...
def match_mentioned_concepts(sents, answers, qids, num_processes):
    res = []
    with Pool(num_processes) as p:
        res = list(tqdm(p.imap(ground_qa_pair, zip(sents, answers, qids)), total=len(sents)))
    return res
...
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=False):
    ...
    res = match_mentioned_concepts(sents, answers, qids, num_processes)
...
if __name__ == "__main__":
    ...
    ground("../data/protoqa/statement/train.statement.jsonl", "../data/cpnet/concept.txt", "../data/cpnet/matcher_patterns.json", "../data/protoqa/grounded/train.grounded.qid.jsonl", 48, False)
