...
relation2id = None
id2relation = None
id2conceptMapping = None
id2relationMapping = None
...
def load_resources(cpnet_vocab_path):
    global concept2id, id2concept, relation2id, id2relation, id2conceptMapping, id2relationMapping

    with open(cpnet_vocab_path, "r", encoding="utf8") as fin:
        id2concept = [w.strip() for w in fin]
    concept2id = {w: i for i, w in enumerate(id2concept)}
    id2conceptMapping = {v:k for k,v in concept2id.items()}

    id2relation = merged_relations
    relation2id = {r: i for i, r in enumerate(id2relation)}
    id2relationMapping = {v:k for k,v in relation2id.items()}
...
def round_triples(sub, relation, obj):
    linker = ""
    if relation == "antonym":
        linker = " is an antonym of "
    elif relation == "relatedto":
        linker = " is related to "
    elif relation == "atlocation":
        linker = " is at location "
    elif relation == "isa":
        linker = " is a "
    elif relation == "hassubevent":
        linker = " has a subevent of"
    elif relation == "partof":
        linker = " is a part of "
    else:
        linker = " is "
    return sub + linker + obj



def load_protoqa_sparse_adj_data_with_contextnode(adj_pk_path, max_node_sum, num_choice, args, baseline="triples"):
    with open(adj_pk_path, 'rb') as fin:
        adj_concept_pairs = pickle.load(fin)

    out = open('../data/protoqa/triples/{}.dev.10.jsonl'.format(baseline), 'w')

    n_samples = len(adj_concept_pairs)
    load_resources('../data/cpnet/concept.txt')
    load_cpnet('../data/cpnet/conceptnet.en.pruned.graph')

    # To store the triples: The format would be {"qid": {"ans1": [triples], "ans2": [triples]}}
    question_to_triples = defaultdict(lambda : defaultdict(list))

    num_questions = 0
    

    for idx, _data in tqdm(enumerate(adj_concept_pairs), total=n_samples, desc='loading adj matrices'):
        adj, concepts, qm, am, cid2score, qid, ans = _data['adj'], _data['concepts'], _data['qmask'], _data['amask'], _data['cid2score'], _data['qid'], _data['ans']
        words = [id2conceptMapping[w] for w in concepts]
        qconcept_ids = [w for i,w in enumerate(concepts) if qm[i]]
        aconcept_ids = [w for i,w in enumerate(concepts) if am[i]]

        filtered_concepts = set()
        half_concepts_length = len(concepts)/10

        # Try to use only first half of the concepts as there are a lot of concepts with low relevancy to the QA context.
        c = 0
        for k, v in cid2score.items():
            if k == -1: continue
            if c == half_concepts_length: break
            c+=1
            filtered_concepts.add(k)
        
        triples_set = set()
        # Iterate over the adj matrix to get the triples
        for row, col, value in zip(adj.row, adj.col, adj.data):
            s_c,t_c  = concept2id[words[row%len(words)]], concept2id[words[col]]
            if s_c not in filtered_concepts or t_c not in filtered_concepts:
                continue
            if cpnet.has_edge(s_c, t_c):
                for e_attr in cpnet[s_c][t_c].values():
                    if e_attr['rel'] >= 0 and e_attr['rel'] < 17:
                        score_val = (cid2score[s_c],cid2score[t_c])
                        if ans == "":
                            ans = "__validationset__"
                        if baseline == 'triples':
                            grounded_triple = round_triples(words[row%len(words)], id2relation[e_attr['rel']], words[col])
                            if grounded_triple not in triples_set:
                                question_to_triples[qid][ans].append(grounded_triple)
                                triples_set.add(grounded_triple)
                        elif baseline == 'sentence':
                            sentence = e_attr['sentence']
                            if not sentence:
                                sentence = round_triples(words[row%len(words)], id2relation[e_attr['rel']], words[col])
                            if sentence not in triples_set:
                                question_to_triples[qid][ans].append(sentence)
                                triples_set.add(sentence)
                            pass
                        elif baseline == 'wikipedia':
                            # if s_c in qconcept_ids and t_c in qconcept_ids:
                            sentence = "___".join([words[row%len(words)], words[col]])
                            if sentence not in triples_set:
                                question_to_triples[qid][ans].append(sentence)
                                triples_set.add(sentence)
                            # raise Exception("Wikipedia baseline not implemented yet")
                        else:
                            raise Exception("needs to pass atleast one baseline parameter")
                        
    for key in question_to_triples:
        json.dump({key: question_to_triples[key]}, out)
        out.write('\n')
