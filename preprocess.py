...
from utils.convert_protoqa import convert_to_entailment_protoqa
...
input_paths = {
    ...
    'protoqa': {
        'dev': './data/protoqa/dev.jsonl',
        'train': './data/protoqa/train.jsonl',
        # 'test': './data/protoqa/test.jsonl'
    }
}
...
output_paths = {
    ...
    'protoqa': {
        'statement': {
                'train': './data/protoqa/statement/train.statement.jsonl',
                'dev': './data/protoqa/statement/dev.statement.jsonl',
                # 'test': './data/protoqa/statement/test.statement.jsonl',
            },
            'grounded': {
                'train': './data/protoqa/grounded/train.grounded.jsonl',
                'dev': './data/protoqa/grounded/dev.grounded.jsonl',
                # 'test': './data/protoqa/grounded/test.grounded.jsonl',
            },
            'graph': {
                'adj-train': './data/protoqa/graph/train_new.graph.adj.pk',
                'adj-dev': './data/protoqa/graph/dev.graph.adj.pk',
                # 'adj-test': './data/protoqa/graph/test.graph.adj.pk',
            },
    },
...
def main():
    ...
    parser.add_argument('--run', default=['common'], choices=['common', 'protoqa', 'csqa', 'hswag', 'anli', 'exp', 'scitail', 'phys', 'socialiqa', 'obqa', 'obqa-fact', 'make_word_vocab'], nargs='+')
    ...
    routines = {
        ...
        'protoqa': [
            {'func': convert_to_entailment_protoqa, 'args': (input_paths['protoqa']['train'], output_paths['protoqa']['statement']['train'])},
            {'func': convert_to_entailment_protoqa, 'args': (input_paths['protoqa']['dev'], output_paths['protoqa']['statement']['dev'])},
            # {'func': convert_protoqa_to_entailment, 'args': (input_paths['protoqa']['test'], output_paths['protoqa']['statement']['test'])},
            {'func': ground, 'args': (output_paths['protoqa']['statement']['train'], output_paths['cpnet']['vocab'],
                                      output_paths['cpnet']['patterns'], output_paths['protoqa']['grounded']['train'], args.nprocs)},
            {'func': ground, 'args': (output_paths['protoqa']['statement']['dev'], output_paths['cpnet']['vocab'],
                                      output_paths['cpnet']['patterns'], output_paths['protoqa']['grounded']['dev'], args.nprocs)},
            # {'func': ground, 'args': (output_paths['protoqa']['statement']['test'], output_paths['cpnet']['vocab'],
            #                           output_paths['cpnet']['patterns'], output_paths['protoqa']['grounded']['test'], args.nprocs)},
            {'func': generate_adj_data_from_grounded_concepts__use_LM, 'args': (output_paths['protoqa']['grounded']['train'], output_paths['cpnet']['pruned-graph'], output_paths['cpnet']['vocab'], output_paths['protoqa']['graph']['adj-train'], args.nprocs)},
            {'func': generate_adj_data_from_grounded_concepts__use_LM, 'args': (output_paths['protoqa']['grounded']['dev'], output_paths['cpnet']['pruned-graph'], output_paths['cpnet']['vocab'], output_paths['protoqa']['graph']['adj-dev'], args.nprocs)},
            # {'func': generate_adj_data_from_grounded_concepts__use_LM, 'args': (output_paths['protoqa']['grounded']['test'], output_paths['cpnet']['pruned-graph'], output_paths['cpnet']['vocab'], output_paths['protoqa']['graph']['adj-test'], args.nprocs)},
        ],
        'csqa': [
